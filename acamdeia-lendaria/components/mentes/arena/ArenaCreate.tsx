...
import { CloneCardSelect } from './CloneCardSelect';
...
export const ArenaCreate: React.FC<ArenaCreateProps> = ({ onBack, onStart }) => {
    ...
    const handleRandomize = () => {
        ...
        let interval = setInterval(() => {
            const r1 = CLONES[Math.floor(Math.random() * CLONES.length)].id;
            const r2 = CLONES[Math.floor(Math.random() * CLONES.length)].id;
        ...
        setTimeout(() => {
            ...
            const final1 = CLONES[Math.floor(Math.random() * CLONES.length)];
            let final2 = CLONES[Math.floor(Math.random() * CLONES.length)];
            while (final2.id === final1.id) {
                final2 = CLONES[Math.floor(Math.random() * CLONES.length)];
            }
    ...
    const c1 = CLONES.find(c => c.id === selectedClone1);
    const c2 = CLONES.find(c => c.id === selectedClone2);
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-12">
             ...
             <div className="flex items-center justify-between">
                 ...
                 <Button variant="outline" size="sm" onClick={handleRandomize} disabled={isRandomizing} className="border-brand-gold/30 text-brand-gold hover:bg-brand-gold/10">
             ...
             <div className="relative grid grid-cols-1 lg:grid-cols-5 gap-4 items-stretch min-h-[400px]">
                 ...
                 <div className="lg:col-span-2 flex flex-col gap-4">
                     <div className={cn(
                         ...
                         selectedClone1 ? "border-brand-cyan/50 shadow-[0_0_30px_-10px_rgba(34,211,238,0.2)]" : "border-white/5 border-dashed"
                     )}>
                         {c1 ? (
                             <>
                                <div className="absolute inset-0 bg-gradient-to-b from-brand-cyan/5 to-transparent pointer-events-none"></div>
                                <div className="w-32 h-32 rounded-full border-4 border-brand-cyan/20 flex items-center justify-center bg-zinc-900 mb-4 shadow-xl z-10">
                                     <span className={cn("text-4xl font-black", c1.color)}>{c1.avatar}</span>
                                </div>
                                <h2 className="text-3xl font-black text-white uppercase tracking-tight z-10">{c1.name}</h2>
                                <p className="text-zinc-400 font-serif z-10">{c1.role}</p>
                                <Badge className="mt-4 bg-brand-cyan/10 text-brand-cyan border-brand-cyan/20">PROPOSIÇÃO</Badge>
                             </>
                         ) : (
                 ...
                 <div className="lg:col-span-1 flex flex-col items-center justify-center relative py-4 lg:py-0">
                     ...
                     <div className="relative z-10 bg-[#050505] p-4 rounded-full border-4 border-zinc-800 shadow-2xl">
                         <span className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-brand-cyan via-white to-red-500 italic pr-1">VS</span>
                     </div>
                 </div>
                     ...
                     <div className={cn(
                         ...
                         {c2 ? (
                             <>
                                ...
                                <div className="w-32 h-32 rounded-full border-4 border-red-500/20 flex items-center justify-center bg-zinc-900 mb-4 shadow-xl z-10">
                                     <span className={cn("text-4xl font-black", c2.color)}>{c2.avatar}</span>
                                </div>
                                <h2 className="text-3xl font-black text-white uppercase tracking-tight z-10">{c2.name}</h2>
                 ...
                 <div className="flex flex-col justify-end space-y-4">
                     <div className="space-y-2">
                         ...
                         <Input 
                            ...
                            className="bg-[#0a0a0a] border-white/10 h-12 text-center font-serif text-lg focus:border-brand-gold"
                            value={topic}
                     ...
                     <div className="space-y-2">
                         ...
                         <Select 
                            ...
                            className="bg-[#0a0a0a] border-white/10 h-10 text-center"
                         />
                     </div>
                     <Button 
                        size="lg" 
